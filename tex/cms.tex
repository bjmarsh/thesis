...
are limited in energy, as the energy radiated by an charged particle traveling in a circular path falls as 
...
corrections. In total there are nearly 10,000 individual magnets. In order to maintain such a
...
data is disentangling the interesting event from the ${\sim}40$ other interactions
...
designed to study a wide range of physics. 
...
\begin{figure}[t]
  \begin{center}
    ...
    \caption{(left) The author next to an opened CMS detector in summer 2014.
      ...
      inside the barrel for scale. (Image via CERN)
            }
...
making for fast, well-defined pulses.
...
and LHC luminosity.
Recording full detector information from a single bunch crossing
...
the HLT. So the trigger path for these events is prescaled by a factor of $\sim$4000,
...
A typical {RAW} event from the JetHT data set is 770 kB.
...
\item NANOAOD: a newly introduced format, consisting of a lightweight columnar data set built from MINIAOD. Much low-level
...
are generated with an event generator such as \textsc{MadGraph}~\cite{madgraph}; parton showering, fragmentation, and decays
...
producing the same data formats as above. For certain samples where computation time is prohibitive
(usually for BSM models), CMS has developed a fast simulation (``FastSim'') framework~\cite{CMS:fastsim} that forgoes
the full \textsc{Geant4} simulation in favor of a much faster parametric approach.
Separate simulation is generated for each year of data collection (2016, 2017, and 2018 for the analysis
presented here), as pileup and detector conditions change from year to year.
...
ill-equipped to handle this many simultaneous collisions, as the increased spatial overlap of
...
\begin{figure}[t]
  \begin{center}
    ...
    \caption{(left) Distribution of line density of PU vertices for different $\langle N_\mrm{PU}\rangle$ scenarios.
      ...
      of vertices. (center) The mean number of PU tracks associated to the primary vertex as a function of line density,
...
spatially overlapping vertices. Pileup interactions have not only a spatial spread with an RMS of around 4.3~cm,
